...
@account_bp.route('/account', methods=['POST', 'PUT', 'GET', 'DELETE'])
def account_route():
    if request.method == 'GET':
        account_no = request.args.get("account_no")
        if account_no:
        ...
        body = request.json
        create_account(body["customer_id"], body["account_no"], body["Balance"])
        return "Account details added successfully"

    elif request.method == 'PUT':
        body = request.json
        update_balance_by_accountid(body["account_id"], body["Balance"])
        return "Account detail updated"

    else:
        body = request.json
        count = delete_account(body["account_id"])
        return "{} rows were affected".format(count)
